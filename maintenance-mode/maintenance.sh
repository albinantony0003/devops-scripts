...
send_slack_notification() {
    if [ "${NOTIFY_TEAM}" = "TRUE" ]; then
        if [ -z "${SLACK_WEBHOOK_URL}" ]; then
            ...
            if [ "${ACTION}" = "ENABLE" ]; then
                COLOR="#8B0000" # Dark Red for down/maintenance mode
            else
                COLOR="#1E4620" # Dark Green for online mode
            fi

            # Format UTC timestamp matching the requested format: YYYY-MM-DD HH:MM:SS UTC
            TIMESTAMP=$(date -u "+%Y-%m-%d %H:%M:%S UTC")

            # Construct attachment message text based on action
            # Construct attachment message text based on action
            if [ "${ACTION}" = "ENABLE" ]; then
                TEXT_CONTENT="🚀*Production Release Alert*
• *Release initiated by DevOps Support Level 3 at \`${TIMESTAMP}\`*
• *Admin Frontend is now in Maintenance Mode👨‍🔧*"
            else
                TEXT_CONTENT="🚀*Production Release Alert*
• *Traffic switch completed successfully at \`${TIMESTAMP}\`*
• *Admin Frontend is back online🟢*"
            fi
...
# --- Execution Block ---
# Execution pattern varies by ACTION (ENABLE vs DISABLE) to guarantee safety
if [ "${ACTION}" = "ENABLE" ]; then
    # 1. Update AWS CloudFront Cache Behavior first to prevent users from sending writes
    update_cloudfront_cdn
    # 2. Update Kubernetes Deployment Environment Variables to Reader
    update_kubernetes_resources
    # 3. Create CloudFront Cache Invalidation
    purge_cdn_edge_caches
    # 4. Notify team
    send_slack_notification
else
    # 1. Update Kubernetes Deployment Environment Variables to Writer first
    update_kubernetes_resources
    # 2. Update AWS CloudFront Cache Behavior to re-enable traffic
    update_cloudfront_cdn
    # 3. Create CloudFront Cache Invalidation
    purge_cdn_edge_caches
    # 4. Notify team
